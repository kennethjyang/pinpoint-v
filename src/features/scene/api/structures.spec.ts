...
describe("syncStructuresVisibility", () => {
  ...
  it("gives each structure its own alpha rather than collapsing on a shared key", async () => {
    ...
    await syncStructuresVisibility(scene, atlas, [alwaysPresent, visible], [visible], FADED_ALPHA);
  ...
  it("finishes and disposes every failing structure's placeholder even when a sibling also fails, so a later sync retries them", async () => {
    ...
    const syncPromise = syncStructuresVisibility(scene, atlas, [], [failing, succeeding], FADED_ALPHA);
    ...
    mockedGet.mockReset();
    mockedGet.mockResolvedValue({ data: new ArrayBuffer(0) });
    await syncStructuresVisibility(scene, atlas, [], [failing, succeeding], FADED_ALPHA);
  ...
  it("imports missing structures concurrently rather than one at a time", async () => {
    ...
    const syncPromise = syncStructuresVisibility(scene, atlas, [], structures, FADED_ALPHA);
  ...
  it("imports a structure only once when two syncs overlap for it", async () => {
    ...
    const first = syncStructuresVisibility(scene, atlas, [], [structure], FADED_ALPHA);
    await Promise.resolve(); // let the first sync claim its placeholder mesh
    const second = syncStructuresVisibility(scene, atlas, [], [structure], FADED_ALPHA);
  ...
  it("sets a faded structure's alpha before its geometry has loaded", async () => {
    ...
    const syncPromise = syncStructuresVisibility(scene, atlas, [structure], [], FADED_ALPHA);
    await Promise.resolve();
  ...
  it("disposes the previous atlas's mesh and rebuilds it when the atlas changes", async () => {
    ...
    await syncStructuresVisibility(scene, humanAtlas, [], [structure], FADED_ALPHA);
